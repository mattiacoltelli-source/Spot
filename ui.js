...
// Guard per evitare crash se APP_UTILS non è ancora pronto al primo micro-caricamento
function esc(v) {
    return window.APP_UTILS ? window.APP_UTILS.escapeHtml(v) : String(v);
}
function isFavorite(id) {
    return window.APP_UTILS ? window.APP_UTILS.isFavorite(id) : false;
}
...
function pretty(value) {
    ...
    const zoneEntry = zones.find(z => z.id === val); 
    if (zoneEntry) return zoneEntry.label; 
    
    const activities = APP_SPOTS.activities || []; 
    const actEntry = activities.find(a => a.id === val); 
    ...
    const map = { 
        ...
        epico: "Epico", iconico: "Iconico", mistico: "Mistico", avventura: "Avventura", 
        "relax wow": "Relax wow", "grande panorama": "Grande panorama", "panorama pulito": "Panorama pulito" 
    }; 
    return map[val] || val; 
}
...
function getSpotCounts() {
    const spots = window.APP_UTILS ? window.APP_UTILS.getAllSpotsWithMeta() : [];
...
function getDistanceLabel(spot) {
    if (!spot || !window.APP_UTILS) return "";
...
function getBestPracticalLine(spot) {
    ...
    if (spot.experience?.mood) parts.push(displayValue(spot.experience.mood));
...
function getClosestPracticalLine(spot) {
    ...
    const parts = [];
    if (spot.zone) parts.push(displayValue(spot.zone));
    if (spot.activity) parts.push(displayValue(spot.activity));
    if (spot.experience?.tempo) parts.push(spot.experience.tempo);
...
function buildTravelQuickCards(app) {
    ...
    const closestSpot = (app._nearbyCache && app._nearbyCache.length > 0) ? app._nearbyCache[0] : window.APP_UTILS.getClosestSpot(); 
    ...
    <div class="quick-card glass best tap" data-quick-id="${bestNow ? esc(bestNow.id) : ""}"> 
        ...
        ${(alt1 || alt2) ? ` 
        <div class="quick-desc" style="margin-top:12px"> Alternative: ${[alt1?.name, alt2?.name].filter(Boolean).map(esc).join(" · ")} </div> ` : ""} 
    </div> 
    <div class="quick-card glass tap" data-quick-id="${closestSpot ? esc(closestSpot.id) : ""}"> 
        ...
        <div class="sunset-chip-row"> 
            ...
            ${closestSpot?.zone ? `<div class="mini-chip gold">${esc(displayValue(closestSpot.zone))}</div>` : ""} 
...
function renderQuickGrid(app) {
    const box = $("quickGrid");
    if (!box || !window.APP_UTILS) return;
    ...
    box.querySelectorAll("[data-quick-id]").forEach(card => {
        card.onclick = () => {
            ...
            if (!id) return;
            const spot = window.APP_UTILS.getAllSpotsWithMeta().find(s => s.id === id);
...
function renderHourly(app) {
    ...
    if (!app.hourlyData || !app.hourlyData.length) { 
    ...
    if (main) { 
        main.textContent = app.mode === "sail" ? `Finestra letta: ${hh} · vento, direzione e onde` : `Finestra letta: ${hh} · prossime 12 ore`; 
    } 
    if (sub) { 
        sub.textContent = app.mode === "sail" ? "In Sail mode leggi soprattutto vento, direzione e altezza onde." : "Lettura rapida di meteo e comfort delle prossime ore."; 
    } 
    ...
    strip.innerHTML = app.hourlyData.map(item => { 
        ...
        <div class="hour-pill ${mood.cls}">${mood.label}</div> ` : `<div class="hour-pill ${mood.cls}">${mood.label}</div>`; 
...
function getAvailableZones() {
    ...
    const allSpots = window.APP_UTILS ? window.APP_UTILS.getAllSpotsWithMeta() : [];
    const ids = [...new Set(allSpots.flatMap(s => Array.isArray(s.zone) ? s.zone : [s.zone]).filter(Boolean))].sort();
...
function getAvailableActivities() {
    ...
    const allSpots = window.APP_UTILS ? window.APP_UTILS.getAllSpotsWithMeta() : [];
    const ids = [...new Set(allSpots.flatMap(s => Array.isArray(s.activity) ? s.activity : [s.activity]).filter(Boolean))].sort();
    return ids.map(id => ({ id, label: id.charAt(0).toUpperCase() + id.slice(1) }));
}
...
function renderFilterBars(app) {
    ...
    if (mapQuickFilters) { 
        ...
        mapQuickFilters.querySelectorAll("[data-mapquick]").forEach(btn => { 
            btn.onclick = () => { app.mapQuickFilter = btn.dataset.mapquick; UI.smartRender(app); }; 
        }); 
    } 
    ...
    if (levelChips) { 
        ...
        levelChips.querySelectorAll("[data-level]").forEach(btn => { 
            btn.onclick = () => { app.level = btn.dataset.level; UI.smartRender(app); }; 
        }); 
    } 
    ...
    if (lightChips) { 
        ...
        lightChips.querySelectorAll("[data-light]").forEach(btn => { 
            btn.onclick = () => { app.light = btn.dataset.light; UI.smartRender(app); }; 
        }); 
    } 
    
    if (zoneChips) { 
        const zones = getAvailableZones(); 
        zoneChips.innerHTML = `<button class="chip ${app.zone === "all" ? "active" : ""}" data-zone="all" type="button">Tutte le zone</button>` + 
        ...
        zoneChips.querySelectorAll("[data-zone]").forEach(btn => { 
            btn.onclick = () => { app.zone = btn.dataset.zone; UI.smartRender(app); }; 
        }); 
    } 
    
    if (activityChips) { 
        const activities = getAvailableActivities(); 
        activityChips.innerHTML = `<button class="chip ${app.activity === "all" ? "active" : ""}" data-activity="all" type="button">Tutte</button>` + 
        ...
        activityChips.querySelectorAll("[data-activity]").forEach(btn => { 
            btn.onclick = () => { app.activity = btn.dataset.activity; UI.smartRender(app); }; 
        }); 
    } 
    ...
    if (favoriteChips) { 
        ...
        favoriteChips.querySelectorAll("[data-favoritesfilter]").forEach(btn => { 
            btn.onclick = () => { app.favoritesFilter = btn.dataset.favoritesfilter; UI.smartRender(app); }; 
        }); 
    } 
    ...
    if (sailChips) { 
        ...
        sailChips.querySelectorAll("[data-sailfilter]").forEach(btn => { 
            btn.onclick = () => { app.sailFilter = btn.dataset.sailfilter; UI.smartRender(app); }; 
        }); 
    } 
}
...
function renderTopLists(app) {
    ...
    if (!wowBox || !sunsetBox || !window.APP_UTILS) return;

    const allSpots = window.APP_UTILS.getAllSpotsWithMeta();
    let wowSpots = []; 
    ...
    if (APP_SPOTS.topSunsetNames?.length) { 
        ...
        sunsetSpots = [...allSpots] 
            .filter(s => { 
                const l = s.light; 
                const lights = Array.isArray(l) ? l : [l]; 
                return lights.some(val => { 
                    const low = String(val || "").toLowerCase(); 
                    return low === "tramonto" || low === "sera"; 
                }); 
            }) 
            .sort((a, b) => (b.experience?.wow || 0) - (a.experience?.wow || 0)) 
            .slice(0, 10); 
    ...
    const metaById = new Map(allSpots.map(s => [s.id, s])); 
        ...
        <div class="featured-card tap" data-featured-id="${esc(spot.id)}"> 
            ...
            <div class="featured-card-body"> 
                ...
                <div class="featured-card-chips"> 
                    ...
                    <div class="mini-chip blue">${esc(displayValue(spot.activity))}</div> 
                </div> 
            </div> 
        </div> `; 
    }; 
    ...
    [wowBox, sunsetBox].forEach(box => { 
        box.querySelectorAll("[data-featured-id]").forEach(card => { 
            card.onclick = () => { 
                const spot = window.APP_UTILS.getAllSpotsWithMeta().find(s => s.id === card.dataset.featuredId); 
                if (spot) { window.APP_UTILS.showSpotDetail(spot); window.APP_UTILS.switchPage("detail"); } 
            }; 
        }); 
    }); 
...
function renderSpotList(app) {
    ...
    if (note) { 
        if (app.search) { 
            note.textContent = items.length ? `${items.length} risultato${items.length !== 1 ? "i" : ""} per "${app.search}"` : `Nessuno spot trovato per "${app.search}"`; 
        } else { 
        ...
        <div class="spot-card glass tap" data-spot-id="${esc(s.id)}"> 
            <div class="spot-head"> 
                <div> 
                    ...
                    <div class="spot-sub">${esc(displayValue(s.zone))} · ${esc(displayValue(s.activity))} · ${esc(displayValue(s.light))}</div> 
                </div> 
            ...
            <div class="spot-meta"> 
                ...
                ${s.difficulty ? `<span class="tag">${esc(displayValue(s.difficulty))}</span>` : ""} 
...
function renderExperienceSection(spot) {
    ...
    return `<div class="detail-section"> 
                ...
                <div class="detail-grid"> 
                    ${exp.tipo ? `<div><div class="k">Tipo</div><div class="v">${esc(exp.tipo)}</div></div>` : ""} 
                    ${exp.tempo ? `<div><div class="k">Durata</div><div class="v">${esc(exp.tempo)}</div></div>` : ""} 
                    ${exp.mood ? `<div><div class="k">Mood</div><div class="v">${esc(displayValue(exp.mood))}</div></div>` : ""} 
                    ${exp.wow ? `<div><div class="k">Wow</div><div class="v">${esc(String(exp.wow))}/10</div></div>` : ""} 
                </div> 
            </div>`;
} 
...
function renderWhenSection(spot) {
    ...
    return `<div class="detail-section"> 
                <h3>Quando andare</h3> 
                <p>Momento migliore: <strong>${esc(displayValue(spot.whenToGo.best))}</strong>${spot.whenToGo.note ? ` — ${esc(spot.whenToGo.note)}` : ""}</p> 
            </div>`;
}
...
function renderAccessSection(spot) {
    ...
    return `<div class="detail-section"> 
                <h3>Accesso</h3> 
                <div class="detail-grid"> 
                    ${acc.difficolta ? `<div><div class="k">Difficoltà</div><div class="v">${esc(displayValue(acc.difficolta))}</div></div>` : ""} 
                    ${acc.parcheggio ? `<div><div class="k">Parcheggio</div><div class="v">${esc(acc.parcheggio)}</div></div>` : ""} 
                    ${acc.walk ? `<div><div class="k">A piedi</div><div class="v">${esc(acc.walk)}</div></div>` : ""} 
                    ${acc.strada ? `<div><div class="k">Strada</div><div class="v">${esc(acc.strada)}</div></div>` : ""} 
                </div> 
            </div>`;
} 
...
function renderSpotTags(spot) {
    ...
    return `<div class="detail-section"> 
                ...
                <div class="spot-meta" style="margin-top:6px"> 
                    ${tags.map(t => `<span class="tag spot-tag">${esc(t)}</span>`).join("")} 
                    ${alias.map(a => `<span class="tag">${esc(a)}</span>`).join("")} 
                </div> 
            </div>`;
}
...
UI.renderSpotDetail = function (app, rawSpot) {
    ...
    if (!box || !rawSpot || !window.APP_UTILS) return;

    const spot = (rawSpot.weatherFit != null) ? rawSpot : (window.APP_UTILS.getAllSpotsWithMeta().find(s => s.id === rawSpot.id) || rawSpot); 
    ...
    <div class="detail-hero" style="background-image:url('${esc(spot.image || "")}')"> 
        ...
        <div class="detail-hero-inner" style="position:relative;z-index:1"> 
            ...
            <div class="detail-sub">${esc(displayValue(spot.zone))} · ${esc(displayValue(spot.activity))} · ${esc(displayValue(spot.light))}</div> 
        </div> 
    </div> 
    <div class="detail-grid"> 
        <div class="detail-box"> 
            <div class="k">Livello</div> 
            <div class="v">${esc(displayValue(spot.level))}</div> 
        </div> 
        <div class="detail-box"> 
            <div class="k">Difficoltà</div> 
            <div class="v">${esc(displayValue(spot.difficulty || "—"))}</div> 
        </div> 
        ${fit ? ` 
        <div class="detail-box"> 
    ...
    const mapBtn = $("detailMapBtn");
    if (mapBtn) mapBtn.onclick = () => window.APP_UTILS.centerSpot(spot.id);
    
    const favBtn = $("detailFavBtn");
    if (favBtn) favBtn.onclick = () => window.APP_UTILS.toggleFavorite(spot.id);
};
...
UI.renderPlannerBox = function (app) {
    ...
    box.innerHTML = slots.map(slot => { 
        ...
        const spot = spotId ? window.APP_UTILS.getAllSpotsWithMeta().find(s => s.id === spotId) : null; 
    ...
    box.querySelectorAll("[data-clear-slot]").forEach(btn => { 
        btn.onclick = () => window.APP_UTILS.clearPlannerSlot(btn.dataset.clearSlot); 
    }); 
};
...
function renderDataPanel() {
    ...
    if (!target || $("dataPanel") || !window.APP_UTILS) return;
    ...
    const expBtn = $("exportDataBtn");
    if (expBtn) expBtn.onclick = () => { window.APP_UTILS.downloadUserData(); }; 
    
    const impBtn = $("importDataBtn");
    if (impBtn) impBtn.onclick = () => { $("importDataInput")?.click(); }; 
    
    const impInput = $("importDataInput");
...
UI.renderGpsBox = function (app, liveData) {
    ...
    // Rimosso controllo e logica su gpsDistance e gpsPoints per fixare il bug (elementi non presenti)
    if (!gpsSpeed || !gpsHeading) return;
    ...
    if (doFull) { 
        ...
        if ($("weatherAlert")) { 
            if (!app.weatherData) { 
                ...
                $("weatherAlert").textContent = app.marineData ? `Vento ${Math.round(app.weatherData.wind)} km/h · onde ${Number(app.marineData.waveHeight || 0).toFixed(1)} m` : "Lettura vela aggiornata."; 
            } else { 
                $("weatherAlert").className = "alert ok"; 
